...
gen_loss = recon_loss + gen_1_loss
...
opt_g = tf.train.AdamOptimizer(0.0001, beta1=0.5)
opt_g = opt_g.minimize(gen_loss, var_list=gen_var)

opt_d = tf.train.AdamOptimizer(0.0001)
...
for i in tqdm(range(epoch)):
    ...
    for j in range(batch_idx):
        ...
        batch_noise = np.random.uniform(-1, 1, size=[batch_size, 64])
